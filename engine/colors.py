...
COLORS = [c for c in globals().values() if isinstance(c, tuple)]
...
def lerp_color(first, second, position, f=lambda x: x):
    x = math.floor(_lerp(first[0], second[0], position, f))
    y = math.floor(_lerp(first[1], second[1], position, f))
    z = math.floor(_lerp(first[2], second[2], position, f))
    return x, y, z


def _lerp(first, second, position, f):
    return f(position) * (second - first) + first


def randomize_color(color, amount=25):
    return (
        _clamp(color[0] + randrange(-amount, amount)),
        _clamp(color[1] + randrange(-amount, amount)),
        _clamp(color[2] + randrange(-amount, amount))
    )
