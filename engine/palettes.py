import random
from engine.colors import from_hex

# Shamelessly stolen: https://lospec.com/palette-list/fantasy-24
BACKGROUND = (0, 0, 0)
FOILAGE_A = (31, 36, 10)
FOILAGE_B = (57, 87, 28)
FOILAGE_C = (165, 140, 39)
GOLD = (239, 172, 40)
WHITE = (239, 216, 161)
GABRIEL_2_1 = (171, 92, 28)
GABRIEL_2_2 = (24, 63, 57)
GABRIEL_2_3 = (239, 105, 47)
GABRIEL_2_4 = (239, 183, 117)
    # (165, 98, 67)
    # (119, 52, 33),
    # (114, 65, 19),
    # (42, 29, 13),
    # (57, 42, 28),
    # (104, 76, 60),
    # (146, 126, 106),
WATER = (39, 100, 104)
    # (239, 58, 12),
    # (69, 35, 13),
LIGHT_WATER = (60, 159, 156)
HORDELING = (155, 26, 10)
    # (54, 23, 12),
BLOOD = (85, 15, 10)
    # (48, 15, 10)


material = {
    'red': {
        '50': (255, 235, 238),
        '100': (255, 205, 210),
        '200': (239, 154, 154),
        '300': (229, 115, 115),
        '400': (239, 83, 80),
        '500': (244, 67, 54),
        '600': (229, 57, 53),
        '700': (211, 47, 47),
        '800': (198, 40, 40),
        '900': (183, 28, 28),
        'A100': (255, 138, 128),
        'A200': (255, 82, 82),
        'A400': (255, 23, 68),
        'A700': (213, 0, 0)
    },
    'pink': {
        '50': (252, 228, 236),
        '100': (248, 187, 208),
        '200': (244, 143, 177),
        '300': (240, 98, 146),
        '400': (236, 64, 122),
        '500': (233, 30, 99),
        '600': (216, 27, 96),
        '700': (194, 24, 91),
        '800': (173, 20, 87),
        '900': (136, 14, 79),
        'A100': (255, 128, 171),
        'A200': (255, 64, 129),
        'A400': (245, 0, 87),
        'A700': (197, 17, 98)
    },
    'purple': {
        '50': (243, 229, 245),
        '100': (225, 190, 231),
        '200': (206, 147, 216),
        '300': (186, 104, 200),
        '400': (171, 71, 188),
        '500': (156, 39, 176),
        '600': (142, 36, 170),
        '700': (123, 31, 162),
        '800': (106, 27, 154),
        '900': (74, 20, 140),
        'A100': (234, 128, 252),
        'A200': (224, 64, 251),
        'A400': (213, 0, 249),
        'A700': (170, 0, 255)
    },
    'deep purple': {
        '50': (237, 231, 246),
        '100': (209, 196, 233),
        '200': (179, 157, 219),
        '300': (149, 117, 205),
        '400': (126, 87, 194),
        '500': (103, 58, 183),
        '600': (94, 53, 177),
        '700': (81, 45, 168),
        '800': (69, 39, 160),
        '900': (49, 27, 146),
        'A100': (179, 136, 255),
        'A200': (124, 77, 255),
        'A400': (101, 31, 255),
        'A700': (98, 0, 234)
    },
    'indigo': {
        '50': (232, 234, 246),
        '100': (197, 202, 233),
        '200': (159, 168, 218),
        '300': (121, 134, 203),
        '400': (92, 107, 192),
        '500': (63, 81, 181),
        '600': (57, 73, 171),
        '700': (48, 63, 159),
        '800': (40, 53, 147),
        '900': (26, 35, 126),
        'A100': (140, 158, 255),
        'A200': (83, 109, 254),
        'A400': (61, 90, 254),
        'A700': (48, 79, 254)
    },
    'blue': {
        '50': (227, 242, 253),
        '100': (187, 222, 251),
        '200': (144, 202, 249),
        '300': (100, 181, 246),
        '400': (66, 165, 245),
        '500': (33, 150, 243),
        '600': (30, 136, 229),
        '700': (25, 118, 210),
        '800': (21, 101, 192),
        '900': (13, 71, 161),
        'A100': (130, 177, 255),
        'A200': (68, 138, 255),
        'A400': (41, 121, 255),
        'A700': (41, 98, 255)
    },
    'light blue': {
        '50': (225, 245, 254),
        '100': (179, 229, 252),
        '200': (129, 212, 250),
        '300': (79, 195, 247),
        '400': (41, 182, 246),
        '500': (3, 169, 244),
        '600': (3, 155, 229),
        '700': (2, 136, 209),
        '800': (2, 119, 189),
        '900': (1, 87, 155),
        'A100': (128, 216, 255),
        'A200': (64, 196, 255),
        'A400': (0, 176, 255),
        'A700': (0, 145, 234)
    },
    'cyan': {
        '50': (224, 247, 250),
        '100': (178, 235, 242),
        '200': (128, 222, 234),
        '300': (77, 208, 225),
        '400': (38, 198, 218),
        '500': (0, 188, 212),
        '600': (0, 172, 193),
        '700': (0, 151, 167),
        '800': (0, 131, 143),
        '900': (0, 96, 100),
        'A100': (132, 255, 255),
        'A200': (24, 255, 255),
        'A400': (0, 229, 255),
        'A700': (0, 184, 212)
    },
    'teal': {
        '50': (224, 242, 241),
        '100': (178, 223, 219),
        '200': (128, 203, 196),
        '300': (77, 182, 172),
        '400': (38, 166, 154),
        '500': (0, 150, 136),
        '600': (0, 137, 123),
        '700': (0, 121, 107),
        '800': (0, 105, 92),
        '900': (0, 77, 64),
        'A100': (167, 255, 235),
        'A200': (100, 255, 218),
        'A400': (29, 233, 182),
        'A700': (0, 191, 165)
    },
    'green': {
        '50': (232, 245, 233),
        '100': (200, 230, 201),
        '200': (165, 214, 167),
        '300': (129, 199, 132),
        '400': (102, 187, 106),
        '500': (76, 175, 80),
        '600': (67, 160, 71),
        '700': (56, 142, 60),
        '800': (46, 125, 50),
        '900': (27, 94, 32),
        'A100': (185, 246, 202),
        'A200': (105, 240, 174),
        'A400': (0, 230, 118),
        'A700': (0, 200, 83)
    },
    'light green': {
        '50': (241, 248, 233),
        '100': (220, 237, 200),
        '200': (197, 225, 165),
        '300': (174, 213, 129),
        '400': (156, 204, 101),
        '500': (139, 195, 74),
        '600': (124, 179, 66),
        '700': (104, 159, 56),
        '800': (85, 139, 47),
        '900': (51, 105, 30),
        'A100': (204, 255, 144),
        'A200': (178, 255, 89),
        'A400': (118, 255, 3),
        'A700': (100, 221, 23)
    },
    'lime': {
        '50': (249, 251, 231),
        '100': (240, 244, 195),
        '200': (230, 238, 156),
        '300': (220, 231, 117),
        '400': (212, 225, 87),
        '500': (205, 220, 57),
        '600': (192, 202, 51),
        '700': (175, 180, 43),
        '800': (158, 157, 36),
        '900': (130, 119, 23),
        'A100': (244, 255, 129),
        'A200': (238, 255, 65),
        'A400': (198, 255, 0),
        'A700': (174, 234, 0)
    },
    'yellow': {
        '50': (255, 253, 231),
        '100': (255, 249, 196),
        '200': (255, 245, 157),
        '300': (255, 241, 118),
        '400': (255, 238, 88),
        '500': (255, 235, 59),
        '600': (253, 216, 53),
        '700': (251, 192, 45),
        '800': (249, 168, 37),
        '900': (245, 127, 23),
        'A100': (255, 255, 141),
        'A200': (255, 255, 0),
        'A400': (255, 234, 0),
        'A700': (255, 214, 0)
    },
    'amber': {
        '50': (255, 248, 225),
        '100': (255, 236, 179),
        '200': (255, 224, 130),
        '300': (255, 213, 79),
        '400': (255, 202, 40),
        '500': (255, 193, 7),
        '600': (255, 179, 0),
        '700': (255, 160, 0),
        '800': (255, 143, 0),
        '900': (255, 111, 0),
        'A100': (255, 229, 127),
        'A200': (255, 215, 64),
        'A400': (255, 196, 0),
        'A700': (255, 171, 0)
    },
    'orange': {
        '50': (255, 243, 224),
        '100': (255, 224, 178),
        '200': (255, 204, 128),
        '300': (255, 183, 77),
        '400': (255, 167, 38),
        '500': (255, 152, 0),
        '600': (251, 140, 0),
        '700': (245, 124, 0),
        '800': (239, 108, 0),
        '900': (230, 81, 0),
        'A100': (255, 209, 128),
        'A200': (255, 171, 64),
        'A400': (255, 145, 0),
        'A700': (255, 109, 0)
    },
    'deep orange': {
        '50': (251, 233, 231),
        '100': (255, 204, 188),
        '200': (255, 171, 145),
        '300': (255, 138, 101),
        '400': (255, 112, 67),
        '500': (255, 87, 34),
        '600': (244, 81, 30),
        '700': (230, 74, 25),
        '800': (216, 67, 21),
        '900': (191, 54, 12),
        'A100': (255, 158, 128),
        'A200': (255, 110, 64),
        'A400': (255, 61, 0),
        'A700': (221, 44, 0)
    },
    'brown': {
        '50': (239, 235, 233),
        '100': (215, 204, 200),
        '200': (188, 170, 164),
        '300': (161, 136, 127),
        '400': (141, 110, 99),
        '500': (121, 85, 72),
        '600': (109, 76, 65),
        '700': (93, 64, 55),
        '800': (78, 52, 46),
        '900': (62, 39, 35)
    },
    'gray': {
        '50': (250, 250, 250),
        '100': (245, 245, 245),
        '200': (238, 238, 238),
        '300': (224, 224, 224),
        '400': (189, 189, 189),
        '500': (158, 158, 158),
        '600': (117, 117, 117),
        '700': (97, 97, 97),
        '800': (66, 66, 66),
        '900': (33, 33, 33)
    },
    'blue gray': {
        '50': (236, 239, 241),
        '100': (207, 216, 220),
        '200': (176, 190, 197),
        '300': (144, 164, 174),
        '400': (120, 144, 156),
        '500': (96, 125, 139),
        '600': (84, 110, 122),
        '700': (69, 90, 100),
        '800': (55, 71, 79),
        '900': (38, 50, 56)
    },
}

black = (0, 0, 0)
white = (255, 255, 255)


def get_color_from_text(color_text):
    """
    Return a color corresponding to the given text.

    'light green' -> light green 500
    'any light green' -> random light green 50 - 900
    """
    if color_text == 'black':
        return black
    elif color_text == 'white':
        return white

    for i in [
        '50', '100', '200', '300', '400', '500',
        '600', '700', '800', '900', 'A100',
        'A200', 'A400', 'A700'
    ]:
        if i in color_text:
            color_text = color_text.replace(i, '').strip()
            return material[color_text][i]
    return material[color_text]['500']


def get_colors(color_key):
    colors = [v for k, v in material[color_key].items() if 'A' not in k]
    colors = sorted(colors, key=lambda c: sum([c[0]*c[0], c[1]*c[1], c[2]*c[2]]))
    return colors


class Palette:
    def __init__(self):
        self.colors = random.sample(material.keys(), 3)
        self.primary = get_colors(self.colors[0])
        self.secondary = get_colors(self.colors[1])
        self.tertiary = get_colors(self.colors[2])
        self.black = black

    def as_list(self):
        return self.primary + self.secondary + self.tertiary

    def get_display_list(self):
        return self.primary[:3] + self.secondary[3:5] + self.tertiary[5:6]

if __name__ == '__main__':
    for color in [
        '1f240a',
        '39571c',
        'a58c27',
        'efac28',
        'efd8a1',
        'ab5c1c',
        '183f39',
        'ef692f',
        'efb775',
        'a56243',
        '773421',
        '724113',
        '2a1d0d',
        '392a1c',
        '684c3c',
        '927e6a',
        '276468',
        'ef3a0c',
        '45230d',
        '3c9f9c',
        '9b1a0a',
        '36170c',
        '550f0a',
        '300f0a',
    ]:
        print(from_hex('#' + color))